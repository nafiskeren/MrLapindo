...
printf " \______  |__|   \__/\  /  \n"
printf "        \/            \/   \n"
printf " = Paypal Valid Email V.1.69 =\n"
printf "Issued $(date)"
printf "${putih}\n"
}
clear
...
bukanheker_request() {
  ...
  footer="[Slackercode - PayPalValid V15] $(($duration % 60))sec.\n"
...
for (( i = 0; i < "${#mailist[@]}"; i++ )); do
  ...
  bukanheker_request "$username" "$indexer" "$tot" "$targetFolder" "$inputFile" &
...
echo "+==========+ Powered By Bukan Heker +==========+"
paypal.sh
Displaying paypal.sh.
